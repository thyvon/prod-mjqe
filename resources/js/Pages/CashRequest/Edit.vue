<template>
    <Main>
      <Head :title="'Edit Cash Request'" />

      <div class="panel panel-inverse">
        <div class="card shadow">
          <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
            <h4 class="mb-0">Edit Cash Request</h4>
...
const props = defineProps({
  users: Array,
  currentUser: Object,
  cashRequest: Object, // Existing cash request data to edit
});

// Form State (Reactive for easy updates)
const form = reactive({
  request_type: props.cashRequest.request_type || '',
  request_date: props.cashRequest.request_date ? props.cashRequest.request_date.split('T')[0] : '', // Ensure format as yyyy-mm-dd
  user_id: props.cashRequest.user_id || (props.currentUser ? props.currentUser.id : ''),
...
const submitForm = () => {
  Inertia.put(`/cash-request/${props.cashRequest.id}`, form, {
    onSuccess: () => alert('Cash request updated successfully!'),
    onError: (errors) => {
      console.error('Failed to update:', errors);
      alert('Failed to update cash request. Please check your inputs.');
    },
  });
};

// Watch for changes if needed (optional, remove if unnecessary)
watch(
  () => props.cashRequest,
  (newValue) => {
    form.request_date = newValue.request_date ? newValue.request_date.split('T')[0] : '';
    form.request_type = newValue.request_type || '';
    form.user_id = newValue.user_id || '';
    form.position = newValue.position || '';
    form.id_card = newValue.id_card || '';
    form.campus = newValue.campus || '';
    form.division = newValue.division || '';
    form.department = newValue.department || '';
    form.description = newValue.description || '';
    form.currency = newValue.currency || '';
    form.exchange_rate = newValue.exchange_rate || '';
    form.amount = newValue.amount || '';
    form.via = newValue.via || '';
